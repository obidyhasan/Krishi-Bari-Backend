...
const generateDashboardReport = (stats: any, from?: Date, to?: Date): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (logoAdded) {
        doc.fillColor(COLORS.primary).font("Helvetica-Bold").fontSize(22).text("Krishi Bari", 100, 55);
      } else {
        doc.fillColor(COLORS.primary).font("Helvetica-Bold").fontSize(22).text("Krishi Bari", 50, 55);
      }
      ...
      doc.fillColor(COLORS.secondary).font("Helvetica-Bold").fontSize(20)
      ...
      doc.fillColor(COLORS.muted).font("Helvetica").fontSize(9)
        ...
        .text(`Generated on: ${new Date().toLocaleDateString()}`, 300, 95, { align: "right" });
      ...
      const drawKPICard = (x: number, label: string, value: string, sub: string, bgColor: string, textColor: string) => {
        doc.rect(x, gridY, cardWidth, cardHeight).fill(bgColor);
        doc.fillColor(textColor).font("Helvetica-Bold").fontSize(15).text(value, x + 12, gridY + 15);
        doc.fillColor(COLORS.text).font("Helvetica-Bold").fontSize(8).text(label.toUpperCase(), x + 12, gridY + 35);
        doc.fillColor(COLORS.muted).font("Helvetica").fontSize(7).text(sub, x + 12, gridY + 47);
      };

      drawKPICard(50, "Total Revenue", `Tk. ${stats.revenue.total.toFixed(2)}`, `This Month: Tk. ${stats.revenue.thisMonth.toFixed(2)}`, COLORS.light, COLORS.primary);
      drawKPICard(50 + cardWidth + gap, "Total Orders", `${stats.orders.total}`, `Today: ${stats.orders.today} | Pend: ${stats.orders.pending}`, COLORS.light, COLORS.secondary);
      drawKPICard(50 + (cardWidth + gap) * 2, "Customers", `${stats.customers.total}`, `New Today: ${stats.customers.newToday}`, COLORS.light, COLORS.secondary);
      drawKPICard(50 + (cardWidth + gap) * 3, "Products Active", `${stats.products.total}`, `${stats.products.lowStock.length} Low Stock`, COLORS.light, "#b45309");

      // --- Left: Top Selling Products Table ---
      const leftSectionY = 230;
      doc.fillColor(COLORS.secondary).font("Helvetica-Bold").fontSize(12).text("Top Selling Products", 50, leftSectionY);

      doc.rect(50, leftSectionY + 15, 230, 20).fill(COLORS.secondary);
      doc.fillColor(COLORS.white).font("Helvetica-Bold").fontSize(8)
        .text("PRODUCT NAME", 60, leftSectionY + 21)
        .text("QTY SOLD", 220, leftSectionY + 21, { width: 50, align: "right" });
      ...
      stats.topProducts.slice(0, 7).forEach((tp: any, index: number) => {
        ...
        doc.text(tp.product?.name?.split(" ").slice(0, 3).join(" ") ?? `Product #${tp.productId}`, 60, currentLeftY, { width: 155 });
        doc.text(tp._sum.quantity.toString(), 220, currentLeftY, { width: 50, align: "right" });
        currentLeftY += 20;
      });

      // --- Right: Stock Alerts Table ---
      doc.fillColor(COLORS.secondary).font("Helvetica-Bold").fontSize(12).text("Critical Stock Alerts", 310, leftSectionY);
      ...
      stats.products.lowStock.slice(0, 7).forEach((p: any, index: number) => {
        ...
        doc.text(p.name.split(" ").slice(0, 3).join(" "), 380, currentRightY, { width: 110 });
        doc.fillColor(p.stock === 0 ? "#dc2626" : COLORS.text).font("Helvetica-Bold");
        doc.text(p.stock.toString(), 500, currentRightY, { width: 40, align: "right" });
        currentRightY += 20;
      });

      // --- Recent Orders Section (Span full width) ---
      const tableY = Math.max(currentLeftY, currentRightY) + 20;
      doc.fillColor(COLORS.secondary).font("Helvetica-Bold").fontSize(12).text("Recent Transactions Log", 50, tableY);
      ...
      stats.recentOrders.slice(0, 8).forEach((o: any, index: number) => {
        ...
        doc.text(new Date(o.createdAt).toLocaleDateString(), 310, currentTableY);
        doc.font("Helvetica-Bold").text(`Tk. ${o.total.toFixed(2)}`, 380, currentTableY, { width: 80, align: "right" });
      ...
      doc.fillColor(COLORS.muted).font("Helvetica").fontSize(8)
        .text("Krishi Bari Executive Administration System Portal", 50, 760, { align: "center" });
...
const generateSalesReport = (report: any, from: Date, to: Date): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({
        margin: 50,
      ...
      doc.fillColor(COLORS.muted).font("Helvetica").fontSize(9)
        .text(`Date Range: ${from.toLocaleDateString()} to ${to.toLocaleDateString()}`, 250, 80, { align: "right" })
        .text(`Generated on: ${new Date().toLocaleDateString()}`, 250, 95, { align: "right" });
      ...
      // --- Table Section ---
      const tableY = 230;
      doc.fillColor(COLORS.secondary).font("Helvetica-Bold").fontSize(12).text("Daily Sales Volume & Revenue", 50, tableY);
      ...
      doc.fillColor(COLORS.white).font("Helvetica-Bold").fontSize(9)
        ...
        .text("REVENUE GENERATED", 400, tableY + 24, { width: 130, align: "right" });

      let currentY = tableY + 43;
      Object.entries(report.byDate).forEach(([date, v]: [string, any], index: number) => {
        if (index % 2 === 0) {
          doc.rect(50, currentY - 3, 500, 18).fill("#fcfcfc");
          doc.fillColor(COLORS.text);
        }
        doc.fillColor(COLORS.text).font("Helvetica").fontSize(9);
        doc.text(new Date(date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }), 70, currentY);
        doc.text(v.orders.toString(), 230, currentY, { width: 100, align: "center" });
        doc.font("Helvetica-Bold").text(`Tk. ${v.revenue.toFixed(2)}`, 400, currentY, { width: 130, align: "right" });
      ...
      // --- Footer ---
      doc.fillColor(COLORS.muted).font("Helvetica").fontSize(8)
        .text("Krishi Bari Business Performance Dashboard Analytics Report", 50, 760, { align: "center" });
...
const generateInventoryReport = (report: any): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fillColor("#b45309").text(`${report.lowStockCount} Products`, 240, summaryY + 30);
      doc.fillColor("#dc2626").text(`${report.outOfStockCount} Products`, 410, summaryY + 30);

      // --- Critical Out of Stock Section ---
      let currentY = 225;

      if (report.outOfStock.length > 0) {
        doc.fillColor("#dc2626").font("Helvetica-Bold").fontSize(11).text("CRITICAL OUT OF STOCK ITEMS", 50, currentY);
        ...
        doc.fillColor("#991b1b").font("Helvetica-Bold").fontSize(8)
          ...
          .text("CURRENT STOCK", 460, currentY + 20, { width: 80, align: "right" });
        ...
        report.outOfStock.forEach((p: any, index: number) => {
          ...
          doc.fillColor("#dc2626").font("Helvetica-Bold").text(`0 ${p.unit}`, 460, currentY, { width: 80, align: "right" });
      ...
      if (report.lowStock.length > 0) {
        ...
        doc.fillColor("#b45309").font("Helvetica-Bold").fontSize(11).text("LOW STOCK WARNING ITEMS", 50, currentY);
        ...
        report.lowStock.forEach((p: any, index: number) => {
          ...
          doc.text(p.name, 150, currentY, { width: 210 });
          doc.text(p.category?.name || "N/A", 370, currentY);
          doc.fillColor("#b45309").font("Helvetica-Bold").text(`${p.stock} ${p.unit}`, 460, currentY, { width: 80, align: "right" });
      ...
      // --- Footer ---
      doc.fillColor(COLORS.muted).font("Helvetica").fontSize(8)
        .text("Krishi Bari Store Inventory Health Logistics Report", 50, 760, { align: "center" });
...
const generateInvoice = (order: any): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ margin: 50, size: "A4", info: { Title: `Invoice - ${order.orderNumber}`, Author: "Krishi Bari" } });
      const buffers: Buffer[] = [];
      doc.on("data", (chunk) => buffers.push(chunk));
      doc.on("end", () => resolve(Buffer.concat(buffers)));
      doc.on("error", (err) => reject(err));
      ...
      let logoAdded = false;
      if (fs.existsSync(logoPath)) {
        try { doc.image(logoPath, 50, 45, { width: 40 }); logoAdded = true; } catch { /* skip */ }
      }
      if (logoAdded) {
        doc.fillColor(COLORS.primary).font("Helvetica-Bold").fontSize(22).text("Krishi Bari", 100, 55);
      } else {
      ...
      doc.fillColor(COLORS.text).font("Helvetica").fontSize(10)
        .text("Dhaka, Bangladesh", 50, 85)
        .text("support@krishibari.com", 50, 100);
      doc.fillColor(COLORS.secondary).font("Helvetica-Bold").fontSize(28).text("INVOICE", 400, 55, { align: "right" });
      doc.moveDown();

      const orderInfoY = 140;
      doc.rect(50, orderInfoY, 500, 60).fill(COLORS.light);
      doc.fillColor(COLORS.muted).font("Helvetica").fontSize(9)
        .text("ORDER NUMBER", 70, orderInfoY + 15).text("DATE", 200, orderInfoY + 15)
        .text("STATUS", 330, orderInfoY + 15).text("PAYMENT", 450, orderInfoY + 15);
      doc.fillColor(COLORS.secondary).font("Helvetica-Bold").fontSize(10)
        .text(`#${order.orderNumber}`, 70, orderInfoY + 30)
        .text(new Date(order.createdAt).toLocaleDateString(), 200, orderInfoY + 30)
        .text(order.status, 330, orderInfoY + 30)
        .text(order.paymentMethod || "COD", 450, orderInfoY + 30);

      const customerY = 220;
      doc.fillColor(COLORS.muted).font("Helvetica-Bold").fontSize(10).text("BILL TO", 50, customerY).text("SHIP TO", 300, customerY);
      doc.fillColor(COLORS.secondary).font("Helvetica-Bold").fontSize(11)
        .text(order.user.name, 50, customerY + 15).text(order.user.name, 300, customerY + 15);
      doc.fillColor(COLORS.text).font("Helvetica").fontSize(9)
        .text(order.user.email, 50, customerY + 30).text(order.user.phone || "N/A", 50, customerY + 42);
      const addressLine = `${order.address.line1}${order.address.line2 ? ", " + order.address.line2 : ""}`;
      doc.text(addressLine, 300, customerY + 30, { width: 200 })
        .text(`${order.address.upazila.name}, ${order.address.district.name}`, 300, customerY + 55)
        .text(`${order.address.division.name}, Bangladesh`, 300, customerY + 67);
      ...
      const drawRow = (label: string, value: string, y: number, bold = false) => {
        doc.fillColor(bold ? COLORS.secondary : COLORS.muted)
          .font(bold ? "Helvetica-Bold" : "Helvetica").fontSize(10)
          .text(label, 350, y).text(value, 450, y, { width: 80, align: "right" });
      };
      drawRow("Subtotal:", `Tk. ${order.subtotal.toFixed(2)}`, summaryY + 15);
      drawRow("Delivery Fee:", `Tk. ${order.deliveryFee.toFixed(2)}`, summaryY + 35);
      if (order.discount > 0) drawRow("Discount:", `-Tk. ${order.discount.toFixed(2)}`, summaryY + 55);
      ...
      doc.fillColor(COLORS.white).font("Helvetica-Bold").fontSize(12)
        ...
        .text(`Tk. ${order.total.toFixed(2)}`, 450, finalTotalY, { width: 80, align: "right" });

      doc.fillColor(COLORS.muted).font("Helvetica").fontSize(8)
        .text("Thank you for shopping with Krishi Bari!", 50, 750, { align: "center" })
